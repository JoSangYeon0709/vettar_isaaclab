...
본 디렉토리를 그대로 git 리포지토리로 만들어 공유하면 됩니다:

```powershell
cd C:\Users\JSY\Desktop\Project\vettar_isaaclab
git init
git add .
git commit -m "init: vettar isaac lab project"
git remote add origin <YOUR_GIT_REMOTE>
git push -u origin main
```

받는 쪽에서 Isaac Sim 5.1 + Isaac Lab 2.3 만 설치돼 있으면 `pip install -e ./source/vettar` 한 줄로 환경이 잡힙니다.

USD 파일 경로는 `vettar.VETTAR_RESOURCES_DIR` 기준으로 동적 해석되므로 어느 머신에서 clone 해도 깨지지 않습니다.
